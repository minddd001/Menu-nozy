...
public plugin_precache() { 
	precache_model("models/codmod/v_knife.mdl") 
	precache_model("models/p_knife.mdl") 
	precache_model("models/codmod/v_butcher.mdl")
	precache_model("models/p_knife.mdl")
	precache_model("models/codmod/v_bak.mdl")
	precache_model("models/codmod/p_bak.mdl")
	precache_model("models/codmod/v_pocket.mdl")
	//precache_model("models/p_knife.mdl")
	precache_model("models/codmod/v_machete.mdl") 
	precache_model("models/codmod/p_machete.mdl")
	precache_model("models/codmod/v_knife.mdl")
} 

public display_knife(id) {
	new menuBody[512]
	add(menuBody, 511, "\rWybierz swoj noz!\w^n^n")
	add(menuBody, 511, "1. Noz Generala \y(Wieksze obrazenia/mniejsza predkosc)\w^n")
	add(menuBody, 511, "2. Noz Snajpera \y(Nie slychac jak chodzisz)\w^n")
	add(menuBody, 511, "3. Noz Sapera \y(Mniejsza grawitacja)\w^n")
	add(menuBody, 511, "4. Noz Szturmowca \y(Szybki bieg)\w^n")
	add(menuBody, 511, "5. Noz Piechociarza \y(Regeneracja zycia)\w^n^n")
	add(menuBody, 511, "0. Wyjdz^n")
	
	new keys = ( 1<<0 | 1<<1 | 1<<2 | 1<<3 | 1<<4 | 1<<9 )
	show_menu(id, keys, menuBody, -1, "Noze")
}
...
public SetKnife(id , Knife) {
	...
	{
		case 0: {
			format(vModel,55,"models/codmod/v_knife.mdl")
			format(pModel,55,"models/p_knife.mdl")
		}
		case 1: {
			format(pModel,55,"models/p_knife.mdl")
			format(pModel,55,"models/codmod/v_butcher.mdl")
		}
		case 2: {
			format(vModel,55,"models/codmod/v_bak.mdl")
			format(pModel,55,"models/codmod/p_bak.mdl")
		}
		case 3: {
			format(vModel,55,"models/codmod/v_pocket.mdl")
			format(pModel,55,"models/p_knife.mdl")
		}
		case 4: {
			format(vModel,55,"models/codmod/v_machete.mdl")
			format(pModel,55,"models/codmod/p_machete.mdl")
		}
		case 5: {
			format(vModel,55,"models/codmod/v_knife.mdl")
			format(pModel,55,"models/p_knife.mdl")
...
public task_healing(id) {  
	...
	if (is_user_alive(id) && (health < maxhealth)) { 
		...
		show_hudmessage(id,"<< TRWA LECZENIE >>")
...
public kmodmsg() { 
	...
	client_print(0,print_chat,"[Noze] Napisz /noze aby wybrac swoj noz!")
}  
